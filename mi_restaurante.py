...
def click_boton(numero):
    ...
    visor_calculadora.delete(0, END) #para que elimine el número presionado anteriormente, sino se repite
    ...
    for c in cuadros_comida:
        if variables_comida[x].get() == 1:
            ...
            if cuadros_comida[x].get() == '0':
                cuadros_comida[x].delete(0, END) #borra la info anterior
    ...
    for c in cuadros_bebida:
        if variables_bebida[x].get() == 1:
            ...
            if cuadros_bebida[x].get() == '0':
                cuadros_bebida[x].delete(0, END) #borra la info anterior
    ...
    for c in cuadros_postres:
        if variables_postres[x].get() == 1:
            ...
            if cuadros_postres[x].get() == '0':
                cuadros_postres[x].delete(0, END) #borra la info anterior
            cuadros_postres[x].focus() #para que el cursor titile
        else:
...
def guardar():
    info_recibo = texto_recibo.get(1.0, END) #obtengo toda la info desde el comienzo (1.0), hasta el final (end)
    archivo = filedialog.asksaveasfile(mode='w', defaultextension='.txt') # pedir guardar archivo, modo escribir, defaultext convertir a txt
    archivo.write(info_recibo)
    archivo.close()
    messagebox.showinfo('Operación éxitosa', 'Su recibo ha sido generado.')

def resetear():
    texto_recibo.delete(0.1, END)

#VOLVER A CERO RECIBO
...
#INHABILITAR CUADROS PARA ESCRIBIR CANTIDADES
...
#DESACTIVAR CHECKBOX
    for v in variables_comida:
        v.set(0)
    for v.set in variables_bebida:
        v.set(0)
    for v.set in variables_postres:
        v.set(0)
...
aplicacion.geometry('1430x630+0+0')

#evitar maximizar (para q no se pierda el formato q le doy)
...
#color de fondo
aplicacion.config(bg='DarkSeaGreen3')

#ESTRUCTURA DE LA APLICACIÓN
#panel superior
panel_superior = Frame(aplicacion, bd=1, relief=FLAT) #bd=background
panel_superior.pack(side=TOP)

etiqueta_titulo = Label(panel_superior, text='Sistema de Facturación', fg='azure4',
                        font=('Dosis', 40), bg='burlywood', width=27) #fg=foreground. COLOR DE FRENTE

etiqueta_titulo.grid(row=0, column=0)

#panel izquierdo
panel_izq = Frame(aplicacion, bd=1, relief=FLAT) #bd=border
panel_izq.pack(side=LEFT)

#panel inferior
panel_costos = Frame(panel_izq, bd=1, relief=FLAT, bg='azure4', padx=50) #bd=border
panel_costos.pack(side=BOTTOM)

#panel costado
panel_comidas = LabelFrame(panel_izq, text='Comida', font=('Dosis', 15, 'bold'),
                           bd=1, relief=FLAT, fg='azure4')
panel_comidas.pack(side=LEFT)

#PANEL BEBIDAS
panel_bebidas = LabelFrame(panel_izq, text='Bebida', font=('Dosis', 15, 'bold'),
                           bd=1, relief=FLAT, fg='azure4')
panel_bebidas.pack(side=LEFT)

#PANEL POSTRES
panel_postres = LabelFrame(panel_izq, text='Postre', font=('Dosis', 15, 'bold'),
                           bd=1, relief=FLAT, fg='azure4')
...
#panel CALCULADORA
panel_calculadora = Frame(panel_derecha, bd=1, relief=FLAT, bg='DarkTurquoise')
panel_calculadora.pack()

#panel BOTONES
panel_botones = Frame(panel_derecha, bd=1, relief=FLAT, bg='DarkTurquoise')
panel_botones.pack()

#panel recibo
panel_recibo = Frame(panel_derecha, bd=1, relief=FLAT, bg='DarkTurquoise')
panel_recibo.pack()

#INCORPORAR DATOS

#listas de productos
lista_comidas = ['pollo', 'pizza','kebab','carne asada', 'milanesas', 'empanadas', 'tarta', 'pastel de papas']
lista_bebidas = ['agua', 'vino tinto', 'vino blanco', 'gaseosa', 'gaseosa light', 'cerveza', 'soda', 'exprimido']
lista_postres = ['flan', 'budin de pan', 'helado', 'frutillas con crema', 'panqueque', 'ensalada de frutas', 'brownie']

#LOOP con CHECKBUTTON
#crear variable dinámicamente
variables_comida = []
cuadros_comida = []
texto_comida = []
contador = 0
...
for comida in lista_comidas:
    ...
    texto_comida[contador]=StringVar() # permite modificar el cuadro
...
for bebida in lista_bebidas:
    ...
    texto_bebida[contador]=StringVar() # permite modificar el cuadro
...
for postres in lista_postres:
    ...
    postres = Checkbutton(panel_postres, text=postres.title(), font=('Dosis', 19,'bold'), onvalue=1, offvalue=0, variable=variables_postres[contador],command=revisar_check)
    ...
    texto_postres[contador]=StringVar() # permite modificar el cuadro
...
#IMPUESTO
etiqueta_impuesto = Label(panel_costos, text='Impuesto', font=('Dosis', 12, 'bold'), bg='azure4', fg='white')
...
botones_creados = [] # almacena los botones q se van a crear en el loop
...
for boton in botones_calculadora:
    boton= Button(panel_calculadora, text=boton.title(), font=('Dosis', 14, 'bold'),fg='white', bg='azure4', bd=1, width=8)
...
botones_guardados[13].config(command=borrar) #llama a la función creada para borrar
botones_guardados[12].config(command=obtener_resultado) #llama a la función creada para resultado
